...
\date{CIMPA Summer School, Rabat \\ \today}
...
\begin{frame}{Short vectors in $\L$}
	...
	\foreach \x in {-2,...,3}
		...
		\draw[fill=CharCoalDark, opacity=0.5] (-3.7,-3.5) rectangle (6.5,-2.0) node[pos=.5, color=white, opacity=1,align=left] {
			\large The {\color{Orange}{Shortest Vector Problem} (SVP) } asks to find non-zero $\vvec$  \\ of minimal length. 
		}; 
	\end{scope}
...
\begin{frame}{Practical Algorithms for SVP}
	...
	\begin{itemize}
		...
		\item {\LARGE{Sieving}} \onslide<3->{(Provable/Heuristic)}
		\onslide<3->{
			\Large
			\begin{align*}
...
\begin{columns}[t]
	\begin{column}{0.3\textwidth}
		\vspace{10pt}
		\begin{tikzpicture}[
		state/.style={circle, fill=white, inner sep=1.5pt},
		]
		%\centering
		\onslide<1->{
