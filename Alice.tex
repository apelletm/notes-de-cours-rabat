...
Let us now prove the main part of the attack, namely that there is an algorithm that, given as input an NTRU$_{q,B}$ instance $h$ with very large $q$, finds a trapdoor for $h$ slightly smaller than $\sqrt{q}$. As explained above, the algorithm works by transforming the NTRU instance into a new NTRU instance in a field of smaller degree, and solving the NTRU problem in this smaller dimensional number field. In order to simplify the statement of the algorithm and the theorem, we will restrict ourselves to a power-of-two cyclotomic field $K$. This way, $K$ has many subfields and we know exactly their degrees (there is at least one for each power-of-two $\leq d$). There is no real restriction to extend the algorithm to other number fields with subfields, but the statement would be messier, depending on the available degrees for the subfields of $K$.

\begin{algorithm}
\caption{Finding somewhat short NTRU trapdoors in power-of-two cyclotomic fields}
...
\ENSURE A pair $(f,g) \in R^2$.
\STATE Let $m$ be the smaller power-of-two $\geq d \cdot \frac{\log B}{\log(q^{1/4}/16)}$
\STATE Let $K' = \QQ[X]/(X^m+1)$ (this is a subfield of $K$)
%TODO add in an example the fact that power of two cyclotomic fields are subfields of one another
\STATE Compute $h' = \N_{K/K'}(h) \in K'$
\STATE Compute $M_{h'} \in \O_{K'}^{2 \times 2}$ the NTRU-module associated to $h'$ in $K'$
\STATE Let $\gamma = q^{1/4}$
\STATE $\vec v \leftarrow \text{SVP}_\gamma(\mink_{K'}(M_h))$ \label{step:SVP-NTRU}
\STATE Compute $(g',f') \in \O_{K'}^2$ such that $\mink_{K'}((f',g')) = \vec v$
\STATE Set $g = g' \in \O_K$ and compute $f \in \O_K$ such that $f = gh \bmod q$ and $f = \sum_{i = 0}^{d-1} f_i X^i$ with $f_i \in (-q/2,q]$ for all $i$'s
\RETURN $(f,g)$
\end{algorithmic}
\end{algorithm} 
...
Let $K = \QQ[X]/(X^d+1)$ with $d$ a power-of-two. Let $q >2$ be an integer and $B > 0$ be a real number with $B \leq q^{1/4}/16$. Let $h \in R_q$ be an NTRU$_{q,B}$ instance. Assume that $h$ admits a trapdoor $(f_0,g_0)$ such that $\|\mink_K(f_0)\|, \|\mink_K(g_0)\| \leq B$ and $\|\mink_K(f_0^{-1})\| \leq B$ and $\N_K(f_0)$, $\N_K(g_0)$ are coprime.
On input $(a,B,h)$, Algorithm~\ref{algo:attack-NTRU} outputs $(f,g) \in R\setminus\{(0,0)\}$ such that $g \cdot h = f \bmod q$ and $\|\mink_K(f)\|, \|\mink_K(g)\| \leq \sqrt{q}/8$. This algorithm makes one call to an SVP$_\gamma$ solver in a lattice of dimension $2m$, where $m = 2^{\left\lceil \log_2\left(\frac{d\log B}{\log(q^{1/4}/16)}\right)\right\rceil}$ and $\gamma = q^{1/4}$. Except for the call to the SVP solver, the algorithm runs in polynomial time.
\end{theorem}

The choice of $m$ and $\gamma$ in the Theorem's statement may be a bit hard to apprehend, so let's take a concrete example. Assume that $B$ is a constant and that $q = 2^{\sqrt{d}}$. Then $m \approx \frac{d}{\sqrt{d}} \approx \sqrt{d}$, and $\gamma = 2^{\sqrt{d}/4} \approx 2^{m}$ (ignoring all constants). Solving SVP$_{2^m}$ in a lattice of dimension $m$ can be done in polynomial time using the LLL algorithm, so in this situation, one should expect that our algorithm runs in polynomial time, even including the SVP call.

On the other hand, assume that $B$ and $q$ are both $\poly(d)$. Then $m \approx d$, and $\gamma = \poly(d)$. We are asked to solve SVP$_{\gamma}$ in a lattice of dimension $d$ with approximation factor $\poly(d)$. The best algorithms to do so are exponential in $d$, hence we do not gain anything compared to running lattice reduction algorithms directly on the NTRU module lattice $M_h$.

\begin{proof}
Again, the assertion on the run time follows easily by inspection of the algorithm. One should note that since $B \leq q^{1/4}/16$, then $m \leq d$, and so there is indeed a subfield of $K$ of degree $m$. The existence of $f$ in the last step of the algorithm follows from the fact that $f$ is defined modulo $q$, and so one can reduce it's coefficient in the interval $(-q/2,q]$.

Regarding correctness, we know that $g' \neq 0$. Indeed, we have seen that by definition of the NTRU module $M_{h'}$, any vector $(g',f')$ in this module satisfies $g' \cdot h' = f' \bmod q$. So if $g' = 0$, then we must have $f' = 0 \bmod q$. Moreover, as $(g',f')$ is a solution to SVP$_\gamma$, it must be non-zero, and so $f' \in \O_{K'}$ must be a non-zero multiple of $q$. This implies in particular that $\|\mink_{K'}(f')\| \geq q$. But we will see below that $\|\mink_{K'}(f')\| \leq \sqrt{q}/8$, hence it must be that $g = g'$ is non-zero, i.e., $(f,g) \in \O_K^2 \setminus \{(0,0)\}$.

The condition $gh = f \bmod q$ is satisfied by definition of $f$ in the last step. It remains to upper bound the size of $f$ and $g$.

Let $f_0$ and $g_0$ be as in the theorem's statement.
...
Let us write $g_0' = \N_{K/K'}(g_0)$ and $f_0' = \N_{K/K'}(f_0)$. Recall also the notation $h' = \N_{K/K'}(h)$ from the algorithm. By Lemmas~\ref{lemma:automorphism-preserves-size} and~\ref{lemma:canonical-norm-subfield}, we know that $\|\mink_{K'}(g_0')\| = \sqrt{|H|}^{-1} \cdot \|\mink_{K}(g_0')\| \leq \sqrt{|H|}^{-1} \cdot \|\mink_{K}(g_0)\|^{|H|} \leq \sqrt{|H|}^{-1} \cdot B^{|H|}$. Similarly, $\|\mink_{K'}(f_0')\|\leq \sqrt{|H|}^{-1} \cdot B^{|H|}$. Letting $B' = \sqrt{|H|}^{-1} \cdot B^{|H|}$ we see that $h'$ is an NTRU$_{q,B'}$ instance in $K'$. Now, recall from the algorithm that $m \geq d \cdot \frac{\log B}{\log(q^{1/4}/16)}$, hence we have
\[B' = \sqrt{|H|}^{-1} \cdot B^{d/m} \leq \sqrt{|H|}^{-1} \cdot B^{\frac{\log(q^{1/4}/16)}{\log B}} = \sqrt{|H|}^{-1} \cdot q^{1/4}/16.\]
The rank-2 module $M_{h'} \subseteq \O_{K'}^2$ contains the short non-zero vector $(g',f')$, hence $\lambda_1(\mink_{K'}(M_{h'})) \leq 2B' \leq \sqrt{|H|}^{-1} \cdot q^{1/4}/8$.

Let $\gamma = q^{1/4}$ and $(g', f') \in \O_{K'}^2$ be a solution to SVP$_\gamma$ in $M_{h'}$, as in the theorem statement. Then 
\[\|\mink_K(g')\| = \sqrt{|H|} \|\mink_{K'}(g)\| \leq \sqrt{H} \cdot \gamma \cdot \lambda_1(\mink_{K'}(M_{h'})) \leq \sqrt{q}/8.\]
Similarly, $\|\mink_K(f')\| \leq \sqrt{q}/8$.

We have now proven that $g = g'$ satisfies the upper bound $\|\mink_K(g)\| \leq \sqrt{q}/8$. It remains to prove that the same holds for $f$, as defined in the last step of the algorithm. This is a little technical, so stay with me.
