...
%$\vec y = \mink(\vec b) \in \CC^{dm}$, $\vec x = \mink(\vec b - \vec e) \in \CC^{dm}$, and $M = \Lambda(\vec A)$, we see that $\vec x$ belongs to the module lattice $\mink(M)$ and that $\|\vec y - \vec x\| = \| \mink(\vec e)\| \leq \sqrt{m} \cdot B$.
%If $\sqrt{m} \cdot B$ were significantly smaller than $\lambda_1(\mink(M))$, then $\vec y$ would be a BDD instance in the module $\mink(M)$. To show that this is the case with high probability, one would need a variant of the Minkowski-Hlawka theorem for module lattices, analogue to Theorem~\ref{thm:Minkowski-Hlawka}. We will not prove this in these lecture notes, and we are not aware of a reference where such a statement is made and proven.

%Even though we do not have a formal proof that $\vec y$ is a BDD instance in $\mink(M)$, we still know that it is quite close to the lattice $\mink(M)$ (closer than what one would expect for $\lambda_1(\mink(M))$ for a sufficiently uniform lattice). And finding the close point $\vec x \in \mink(M)$ of $\vec y$ allows to recover the secret $\vec s$ of the MLWE instance. Hence, in a sense, MLWE can be seen as a module-BDD instance, in the module lattice $\mink( \Lambda(\vec A))$ where $\vec A$ is sampled uniformly at random.
