...
pour les
cours avancés (notamment les réseaux $q$-aires). 

Il abordera les théorèmes
...
Minkowski's theorem gives a relationship between the minimum and the determinant of a lattice. 
...
spanned by the columns of the diagonal matrix~$\mbox{diag}(\eps, 1/\eps)$ for~$\eps>0$ going to~$0$. We will prove Minkowski's theorem for the Construction~A lattices, up to a loss of a factor~2. (Recall that their determinants were computed in Lemma~\ref{le:consA_dimdet}.)
